...
//Xử lý list control
// let listSortByName = document.getElementById('list-sort-name');
// let listSortByDeadline = document.getElementById('list-sort-deadline');
let moveList = document.getElementById('move-list');
let copyList = document.getElementById('copy-list');
		...
		case 3:
			for (let i = 0; i < numberOfCard - 1; i++) {
				...
				for (let j = i + 1; j < numberOfCard; j++) {
					//console.log((listCard.children[i].getAttribute('data-card')));
					if(listCard.children[i].getAttribute('data-card') > listCard.children[j].getAttribute('data-card')) {
						console.log("adasd");
						listCard.children[i] = tmp;
						listCard.children[i] = listCard.children[j];
						listCard.children[j] = tmp;
					}
				}
			}
			break;
