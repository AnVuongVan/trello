...
let cardInfo = document.getElementById('window');
let windowHeaderLabel = document.getElementById('window-header-label');
let windowHeaderEdit = document.getElementById('window-header-edit');
let windowCloseBtn = document.getElementsByClassName("window-close-btn")[0];
...
windowHeaderLabel.addEventListener('dblclick', function () {
	swapElement(windowHeaderEdit, windowHeaderLabel);
	windowHeaderEdit.focus();
});

windowHeaderEdit.addEventListener('blur', function () {

	if (windowHeaderEdit.value !== "") {
		windowHeaderLabel.firstElementChild.innerHTML = windowHeaderEdit.value;
...
windowCloseBtn.addEventListener('click', function () {
	...
	windowHeaderEdit.value = "";
	windowHeaderLabel.firstElementChild.innerHTML = "";
	clearDetail();
});
...
function renderCheckList(checklist) {
	...
	checkListControlDelete.addEventListener('click', function () {
		let listControl = document.getElementById('window');
		let url = setURLCard(listControl.getAttribute('data-id-board'), listControl.getAttribute('data-id-list'), listControl.getAttribute('data-id-card'));
		url = url + "/checklists/" + checklist.id;
		let data = setTokenToData("", "");
		handleAPI(url, data, "DELETE", "checklists");
	...
	checkListNameEditSaveBtn.addEventListener('click', function () {
		if (checklistNameEditInput.value != "") {
			...
			handleAPI(url, data, "PUT", "checklists");
	...
	newItemSaveBtn.addEventListener('click', function () {
		if (newItemSectionInput.value != "") {
			...
			let data = setTokenToData("list_checklists", setCheckListToJson(listItems, newItemSectionInput.value));
			
			handleAPI(url, data, "PUT", "checklist");
			//renderSubtask(listItems, id, newItemSectionInput.value, "false");
...
newTodoListAddBtn.addEventListener('click', function () {
	if (newTodoListBoxInput.value != "") {
		...
		handleAPI(url, data, "POST", "checklists");
		///////////////////////////////////////
...
function renderComment(activity, createdDate) {
	...
	//newCommentBadge.innerHTML = "<span>" + activity.member_name[0] + "</span>";
...
function renderCardInfo(data) {
	console.log(data);
	windowHeaderLabel.innerHTML = "<span>" + data.title + "</span>";
	windowHeaderEdit.value = data.title;
	cardInfo.parentElement.classList.toggle('hide');
...
addAttachmentBtn.addEventListener('click', function () {
	if (attachmentSection.classList.contains('hide')) {
		...
		console.log(attachmentList.innerHTML == null);
